"""CPU — pleading-ladder stereo compliance vs assertiveness control.

Compares each pleading profile to the matched assertiveness plain ladder
(sensitivity_smiley_plain_* or sensitivity_*) at the same level index t1–t6.

Supports pleading-native variants and pleading+smiley combos (--with-smileys).

Example:
    uv run python scripts/analyze_pleading_compliance.py --category Gender_identity --with-smileys
"""
...
from mech_interp_bbq.sensitivity import PLEADING_LADDER_VARIANTS, SMILEY_SUFFIX_PROFILES
...
def _analyze_one(
    ...
    pleading = d["ladder_stereo"][:n_use]
    assertive = assert_ladder[:n_use]
    pleading_levels = [str(x) for x in d["ladder_levels"]]

    rates_pl = _level_rates(pleading, stereo_ids)
    rates_as = _level_rates(assertive, stereo_ids)
    delta = [p - a for p, a in zip(rates_pl, rates_as, strict=True)]

    levels_out: dict = {}
    print(f"--- {label}  vs  assertiveness ---")
    print(f"{'level':<6} {'assert':>8} {'plead':>8} {'Δ':>8}")
    for i, (asr, plr, dlt) in enumerate(zip(rates_as, rates_pl, delta, strict=True)):
        lvl = f"t{i + 1}"
        levels_out[lvl] = {
            "assertiveness_compliance": asr,
            "pleading_compliance": plr,
            "delta_pleading_minus_assertiveness": dlt,
            "assertiveness_template": assert_levels[i] if i < len(assert_levels) else None,
            "pleading_template": pleading_levels[i],
        }
        print(f"{lvl:<6} {asr:>7.1%} {plr:>7.1%} {dlt:>+7.1%}")
    ...
    print(f"mean Δ (pleading − assertiveness): {summary['mean_delta_t1_t6']:+.1%}\n")

    return {
        "source": path.name,
        "ladder_levels": pleading_levels,
        "levels": levels_out,
        "summary": summary,
        "n": n_use,
    }
...
def main() -> None:
    ...
    ap.add_argument(
        "--with-smileys",
        action="store_true",
        help="Also analyze pleading+smiley combo caches",
    )
    ap.add_argument(
        "--smiley-variants",
        default="all",
        help=f"Smiley profiles when --with-smileys (default: all). Choices: {', '.join(SMILEY_SUFFIX_PROFILES)}",
    )
    ...
    out: dict = {
        "experiment": "pleading_ladder_compliance",
    ...
    for variant in variants:
        path = cache_dir / f"sensitivity_pleading_{variant}_{args.category}.npz"
        if not path.exists():
            print(f"[skip] missing {path.name}")
            continue
        result = _analyze_one(f"pleading:{variant}", path, assert_ladder, assert_levels, n_assert)
        out["variants"][variant] = result
        out["n"] = result["n"]

    for profile in smiley_profiles:
        path = cache_dir / f"sensitivity_pleading_smiley_{profile}_{args.category}.npz"
        if not path.exists():
            print(f"[skip] missing {path.name}")
            continue
        key = f"smiley_{profile}"
        result = _analyze_one(f"pleading+smiley:{profile}", path, assert_ladder, assert_levels, n_assert)
        out["variants"][key] = result
        out["n"] = result["n"]

    if not out["variants"]:
        raise SystemExit("No pleading caches found — run collect_pleading_ladder.py on GPU first.")
    ...
    out_path = Path(args.out or f"results/pleading_compliance_{args.category}.json")
