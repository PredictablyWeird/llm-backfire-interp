...
For reason-before-answer variant see ``nudge_sensitivity_reasoning.py``.
...
def main() -> None:
    ...
    for direction in ("stereo", "other"):
        ladder = np.stack(
            [run(prompts_for(rows, t, direction), f"ladder/{direction}/t{i+1}")
             for i, t in enumerate(LADDER)], axis=1)        # (n, 6, 3)
        saved[f"ladder_{direction}"] = ladder
        rep = np.stack(
            [run(prompts_for(rows, " ".join([REP_SENT] * k), direction), f"rep/{direction}/k{k}")
