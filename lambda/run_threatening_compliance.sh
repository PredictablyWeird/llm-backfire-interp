#!/usr/bin/env bash
# GPU experiment: threatening ladder vs assertiveness control (Gender default).
#
#   bash lambda/run_threatening_compliance.sh
#
...
echo "============ THREATENING LADDER: collect (variants=$VARIANTS, n=$MAX_EXAMPLES) ============"
run uv run --env-file .env python scripts/collect_threatening_ladder.py "${COLLECT_ARGS[@]}"

echo "============ THREATENING LADDER: analyze (vs assertiveness plain) ============"
run uv run python scripts/analyze_threatening_compliance.py "${ANALYZE_ARGS[@]}"
